...
import { ClipboardList, BookOpen, TrendingUp, BarChart3 } from 'lucide-react';
...
export const GraphView = ({ students, subjects, tests }: GraphViewProps) => {
  const [selectedStudentId, setSelectedStudentId] = useState(students[0]?.id);
  const [selectedSubjectId, setSelectedSubjectId] = useState<string | 'all'>('all');
  const selectedStudent = students.find(s => s.id === selectedStudentId);

  const subjectChartData = selectedStudent ? subjects.map(sub => {
  ...
  const testChartData = selectedStudent ? tests
    ...
    .map(t => {
      ...
      return {
        ...
        score: t.marks[selectedStudent.id] || 0,
        classAvg: parseFloat(classAvg.toFixed(1)),
        max: t.maxMarks,
    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Comparing individual progress against class benchmarks.</p>
        </div>
      </div>
                ...
                <div className="flex gap-8">
                  <div className="text-center">
                    ...
                    <p className="text-2xl font-mono font-bold">
                      {subjects.length > 0 
                        ? (Object.values(selectedStudent.marks).reduce((a, b) => a + b, 0) / subjects.length).toFixed(1)
                        : "0.0"}
                    </p>
                  </div>
                  <div className="text-center">
                              ...
                              return (
                                <div className="bg-white p-3 rounded-xl shadow-xl border border-zinc-100">
                                  ...
                                  <div className="space-y-1">
                                    <div className="flex justify-between gap-4">
                                      <span className="text-xs text-zinc-500">Student:</span>
                                      <span className="text-xs font-bold">{payload[0].value} / {payload[0].payload.max}</span>
                                    </div>
                                    <div className="flex justify-between gap-4">
                                      <span className="text-xs text-zinc-500">Class Avg:</span>
                                      <span className="text-xs font-bold text-zinc-400">{payload[1].value}</span>
